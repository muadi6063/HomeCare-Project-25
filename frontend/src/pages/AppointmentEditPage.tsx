...
// Helper to ensure time is in "HH:MM:SS" format before sending it back to the API
...
const AppointmentEditPage: React.FC = () => {
  ...
  // Load appointment details when the page is opened
  useEffect(() => {
    async function load() {
      try {
        setError("");
  ...
  // Handle form submit: send updated appointment data to backend
  ...
  // First: if we don't have model yet, show loading or error message
  if (!model) {
    return (
  ...
  // We have the model: check authorization (RBAC)
  const canEdit =
    role === "Admin" ||
          ...
          <Form onSubmit={handleSubmit}>
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                onChange={(e) =>
                  setModel({ ...model, taskDescription: e.target.value })
                }
